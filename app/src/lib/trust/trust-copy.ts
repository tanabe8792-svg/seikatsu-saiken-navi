...
export const TRUST_DEVELOPER = {
  ...
  body: [
    ...
    "個人で開発・更新していますが、被害の内容を運営者だけが覗ける形にはしていません。",
  ],
} as const;
...
export const TRUST_INFO_HANDLING = {
  ...
  body: [
    ...
    "このナビは公式サイトを何度も自動で取りに行かず、案内に「情報の更新」日付を載せ、必要なとき公式リンクを開く形にしています。",
  ],
} as const;
...
export const TRUST_PERSONAL_DATA = {
  heading: "個人情報の扱い",
  body: [
    "被害の状況や手続きの進み具合は、あなた自身の情報です。運営者が全員分を一覧で見られる仕組みにはしていません。開発者だけが内容を見られる状態にもしないようにしています。",
    "登録なしで使う場合（標準）：やること・進捗・メモは、この端末（ブラウザ）内に保存されます。運営のサーバーへは自動では送りません。被害写真もこの端末内に残し、自動ではサーバーに送りません。",
    "マイページ登録（メール・LINEログイン）は任意です。別の端末から続きを再開したいときだけ使えます。登録した内容のやりとりは、安全な通信（暗号化）で行います。本人以外は読めないようにし、運営用の「全員分を見る画面」は作っていません。",

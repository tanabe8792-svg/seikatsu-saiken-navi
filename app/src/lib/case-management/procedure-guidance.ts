  ...
  switch (actionId) {
    ...
    case "rw-j04-insurance-report":
      return staticGuidance(
        ...
        "火災保険・地震保険は「事故報告・相談」が入口です。証券番号が分からなくても、会社名が分かれば総合窓口で案内してもらえます。",
        [
          {
            label: "はじめに伝えること",
    ...
    case "rw-j04-life-rebuild":
      return staticGuidance(
        "生活再建支援金の申請案内",
        "国の制度説明と、地域の支援ポータルへ直接進めます。申請窓口は市町村の案内が正です。",
    ...
    case "rw-j05-emergency-repair": {
      ...
      return staticGuidance(
        `${name}の応急・緊急修理 申請案内`,
        "雨の前に確認したい屋根・ブルーシート等の緊急修理と、当面住むための応急修理の公式入口です。県の案内に加え、選んだ市町村の公式も出します。",
        "緊急修理は被害拡大防止（ブルーシート等）、応急修理は当面の居住のための修理、と案内が分かれていることがあります。対象・期限・り災証明の要否は自治体の公式が正です。情報は更新されるので、開いたページの日付も確認してください。",
        [
          {
            label: "大事な注意",
    ...
    case "rw-j02-water-children":
      return staticGuidance(
        "給水・ライフラインの申請案内",
        "給水情報と、水道料金の減免、電気・ガスの安全確認につながる公式入口です。自分で復旧状況を探さなくても進めます。",
        "断水中は給水の確保が先です。あわせて水道減免の届出、停電・ガス停止時の事業者案内を確認すると、あとの手間と機会損失を減らせます。",
        [
          {
            label: "水道・減免",
    ...
    case "rw-j04-programs": {
      ...
      return staticGuidance(
        ...
        "国・県・市町村の支援が並んでいます。自分の状況に合いそうなものを開き、詳細ページの条件を確認してください。対象かの最終判断は公式案内です。",
        [
          {
            label: "見落としやすい確認",
    ...
    case "rw-j05-temp-housing":
      return staticGuidance(
        ...
        "対象条件・必要書類・締切は県・市町村の最新発表が正です。通勤・通学・通院の条件も、申し込み前にメモしておくと後で困りにくいです。",
        [],
        withMunicipalitySite(profile, [
          {
